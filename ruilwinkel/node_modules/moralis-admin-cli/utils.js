...
function getSelectedServer(ownedServers) {
    ...
    const chosenServer = grab('--moralisSubdomain') || grab('-d');
    ...
    if (chosenServer){
        ...
        console.log("Subdomin not found!");   
    }
...
function getGetLocalDevchain() {
    ...
    const chosenChain = grab('--chain') || grab('-c');

    // if so
    if (chosenChain){

        // Filter out all chains that does not match the choice
        supportedDevChains = supportedDevChains.filter(item => item.name.toLowerCase() == chosenChain.toLowerCase());

        // If we have a match then return it
        if (supportedDevChains.length > 0){
            return supportedDevChains[0];
...
/**
 * Gets a list of all the servers owned by the user
 * @returns {Promise}
 */
...
function getSelectedServerRegion() {
    ...
    if (chosenRegion){
        ...
        possibleRegions = possibleRegions.filter(item => item.name.toLowerCase() == chosenRegion.toLowerCase());

        // Return region if a match was found
        if (possibleRegions.length > 0) return possibleRegions[0];
...
function getSelectedServerNetwork() {
    ...
    if (chosenNetwork){
        ...
        possibleNetworks = possibleNetworks.filter(item => item.name.toLowerCase() == chosenNetwork.toLowerCase());

        // Return region if a match was found
        if (possibleNetworks.length > 0) return possibleNetworks[0];
 ...
 */
 function getSelectedServerProviders(network) {
    ...
    if (chosenProviders){
         ...
         // Filter out all networks that does not match the choice
         possibleProviders = possibleProviders.filter(item => item.name.toLowerCase() == chosenProviders.toLowerCase());

        // Return chains if a match was found
        if (possibleProviders.length > 0) return possibleProviders;
 ...
 */
 function getSelectedServerProvider(currentProviders) {
    ...
    if (chosenProvider){
        ...
        // Return chains if a match was found
        if (possibleProviders.length > 0) return possibleProviders[0];
